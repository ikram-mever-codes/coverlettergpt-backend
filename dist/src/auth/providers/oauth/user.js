import { createUser, sanitizeAndSerializeProviderData, validateAndGetUserFields, createProviderId, findAuthWithUserBy, } from 'wasp/auth/utils';
import { prisma } from 'wasp/server';
import { getRedirectUriForOneTimeCode, tokenStore } from 'wasp/server/auth';
import { onBeforeSignupHook, onAfterSignupHook, onBeforeLoginHook, onAfterLoginHook, } from '../../hooks.js';
export async function finishOAuthFlowAndGetRedirectUri({ provider, providerProfile, providerUserId, userSignupFields, req, oauth }) {
...
async function getAuthIdFromProviderDetails({ providerId, providerProfile, userSignupFields, req, oauth, }) {
    ...
    if (existingAuthIdentity) {
        ...
        const userFields = await validateAndGetUserFields({ profile: providerProfile }, userSignupFields);
